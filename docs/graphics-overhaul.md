...
Branch: `amos/couch-graphics` (rebased onto the sessions/auth/benchmark main
after Pandora's #25/#33-#36 infra landed — the couch UI binds its four screens
to her dynamic sessions instead of fixed server slots). Rendering + spectator
UI only — zero server or engine changes.
